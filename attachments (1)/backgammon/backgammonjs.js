...
//const fehlerText = () => t('errors.invalid_input', { defaultValue: "Ungültige Eingabe" });
...
export class SpielLogik {
    constructor(spieler1Objekt, spieler2Objekt) {
        ...
        this.schwarzeSteine = new SpielfeldKomponenten.SteineSchwarz("schwarz_standard", [], "feld_standard", this);//id, pos, fId, this); 
        this.weißeSteine = new SpielfeldKomponenten.SteineWeiß("schwarz_standard", [], "feld_standard", this);//id, pos, fId, this);
        ...
        if (hauptAnzeige && !this.anzeigeSpieler) {
            hauptAnzeige.textContent = `Am Zug: ${this.spieler1.spielerName} (Weiß)`;
        }
...
// Zustandstracking des Matches
                ...
                if (this.wurfSpieler1 === 0) {
                    ...
                    if (this.wurfSpieler1 > this.wurfSpieler2) {
                        setTimeout(() => {   
                            ...
                            defaultValue:"{{name1}} gewinnt den Startwurf und beginnt (Farbe: Weiß)!",
                            playerName:name1});

                            this.aktiverSpielerFarbe = "Weiß";
                            if (this.anzeigeSpieler) this.anzeigeSpieler.textContent = name1 + " (Weiß)";
                            this.phase = "spiel";
                        ...
                        setTimeout(() => {   
                            i18nAlert('alerts.startwinname_message',{
                            defaultValue:"{{name2}} gewinnt den Startwurf und beginnt (Farbe: Weiß)!",
                            playerName:name2});
                            this.aktiverSpielerFarbe = "Weiß";
                            if (this.anzeigeSpieler) this.anzeigeSpieler.textContent = name2 + " (Weiß)";                        
                            this.phase = "spiel";
                            btnRoll.textContent = "Regulär Würfeln";
    ...
    document.querySelectorAll('.point').forEach(zacke => {
        zacke.addEventListener('click', () => {
            ...
            const hatSteineAufBar =    (this.aktiverSpielerFarbe === "Weiß" && this.SpielfeldKomponenten.barWeiß.anzahlSteine > 0) || 
                                       (this.aktiverSpielerFarbe === "Schwarz" && this.SpielfeldKomponenten.barSchwarz.anzahlSteine > 0);
            ...
            // --- NEU: LOGIK FÜR DAS WIEDEREINSETZEN VON DER BAR ---
                ...
                // Schwarz startet nach Feld 24 (quasi bei 25) -> distanz ist 25 - Feldnummer
                    let barDistanz = this.aktiverSpielerFarbe === "Weiß" ? zuId : (25 - zuId);
                // Prüfen, ob die gewürfelten Augen zu diesem Einstiegsfeld passen
                    ...
                    // Prüfen, ob das Feld frei ist (weniger als 2 gegnerische Steine)
                        const gegnerFarbe = this.aktiverSpielerFarbe === "Weiß" ? "Schwarz" : "Weiß";
                    ...
                    // --- ZUG VON DER BAR AUSFÜHREN ---
                        const barZoneId = this.aktiverSpielerFarbe === "Weiß" ? "Bar-Weiß-Zone" : "Bar-Schwarz-Zone";
                        ...
                        // Falls der Gegner dort genau 1 Stein hat -> Schlagen!
                            if (gegnerSteineAnzahl === 1) {
                                ...
                                if (gegnerFarbe === "Weiß") this.SpielfeldKomponenten.barWeiß.anzahlSteine++; else this.SpielfeldKomponenten.barSchwarz.anzahlSteine++;
                            }
                        ...
                        // Zähler verringern
                            if (this.aktiverSpielerFarbe === "Weiß") this.SpielfeldKomponenten.barWeiß.anzahlSteine--; else this.SpielfeldKomponenten.barSchwarz.anzahlSteine--;             
            ...
            // FALL 3: Stein bewegen / Schlagen (Normaler Zug vom Feld)
                ...
                const vonId = parseInt(spiel.ausgewaehlteZacke.getAttribute('data-point'));
                let distanz = this.aktiverSpielerFarbe === "Weiß" ? (zuId - vonId) : (vonId - zuId);
                if (distanz <= 0) {
                    i18nAlert('alerts.wrongdirection_message',{defaultValue:"Falsche Richtung! Du musst dich vorwärts bewegen."});
                    this.aufhebenSelektion(this);
                    return;
                }
                const aktuellerChecker = (this.aktiverSpielerFarbe === "Weiß") ? this.weißeSteine : this.schwarzeSteine;
                aktuellerChecker.fuehreZugAus(distanz, zacke, () => {
                    // Wenn der Zug erfolgreich war, bewegen wir den ausgewählten Stein
                    const steinZumBewegen = spiel.ausgewaehlteZacke.querySelector('.checker.selected');
                    if (steinZumBewegen) {
    ...
    document.querySelectorAll('.point').forEach(zacke => {
        zacke.addEventListener('dblclick', () => {
            if (this.phase !== "spiel" || this.verbleibendeZuege.length === 0) return;
            ...
            if (!obersterStein || !obersterStein.classList.contains(this.aktiverSpielerFarbe)) return;
            ...
            let benoetigteAugen = this.aktiverSpielerFarbe === "Weiß" ? (25 - vonId) : vonId;
            ...
            if (wuerfelIndex === -1) {
                ...
                const hoehererWuerfel = this.verbleibendeZuege.find(w => w > benoetigteAugen);
                if (hoehererWuerfel) {
            ...
            if (wuerfelIndex !== -1) {
                ...
                this.verbleibendeZuege.splice(wuerfelIndex, 1);
                this.aufhebenSelektion(this);
                i18nAlert('alerts.stoneout_message',{farbuebersetzungen : this.aktiverSpielerFarbe});
...
darfAusspielen(spielerFarbe,spiel) {
    ...
    if (spielerFarbe === "Weiß" && this.SpielfeldKomponenten.barWeiß.anzahlSteine  > 0) return false;
    if (spielerFarbe === "Schwarz" && this.SpielfeldKomponenten.barSchwarz.anzahlSteine  > 0) return false;
...
pruefeSpielerWechsel() {
        ...
        const farbZusatz = this.aktiverSpielerFarbe === "Weiß" ? " (Weiß)" : " (Schwarz)";
        ...
        if (this.anzeigeSpieler) {
            ...
            i18nAlert('alerts.changeplayer_message', {
                ...
                playerName: spiel.aktiverSpielerFarbe === "Weiß" ? name1 : name2
            });
