...
export class SpielLogik {
    constructor() {
        ...
        this.checker = new SpielfeldKomponenten.Steine();
        ...
        // Falls du die Feld-Klasse dort auch mal direkt brauchst:
...
//export let highscoreListe = JSON.parse(localStorage.getItem("backgammon_highscores")) || [];
    ...
    // --- A: DIE AUSLOSUNG ---
                ...
                if (wurfSpieler1 === 0) {
                    ...
                    if (wurfSpieler1 > wurfSpieler2) {
                        setTimeout(() => {   
                            ...
                            this.verbleibendeZuege = [wurfSpieler1, wurfSpieler2]; 
                        }, 250);
                    } 
            ...
            // Wenn noch ungenutzte Würfel da sind, darf nicht neu gewürfelt werden
                ...
                SpielaufhebenSelektion();
                const wuerfel1 = Math.floor(Math.random() * 6) + 1;//hier sollte dann sendnum() vonm der classe würfel aufgerufen werden
    ...
    if (btnPass) {
        btnPass.addEventListener("click", () => {
            if (phase === "spiel" && this.verbleibendeZuege.length > 0) {
    ...
    document.querySelectorAll('.point').forEach(zacke => {
        zacke.addEventListener('click', () => {
            ...
            if (!SpielfeldKomponenten.ausgewaehlteZacke) {
            // --- NEU: LOGIK FÜR DAS WIEDEREINSETZEN VON DER BAR ---
                ...
                // Prüfen, ob die gewürfelten Augen zu diesem Einstiegsfeld passen
                    ...
                    // --- ZUG VON DER BAR AUSFÜHREN ---
                        ...
                        // Zähler verringern
                            if (this.aktiverSpielerFarbe === "Weiß") barWeiß--; else barSchwarz--;             
            ...
            else if (SpielfeldKomponenten.ausgewaehlteZacke === zacke) {
                aufhebenSelektion(spiel);
            } 
            // FALL 3: Stein bewegen / Schlagen (Normaler Zug vom Feld)
            else {
                const vonId = parseInt(SpielfeldKomponenten.ausgewaehlteZacke.getAttribute('data-point'));
                ...
                aktuellerChecker.fuehreZugAus(distanz, zacke, () => {
                    ...
                    const steinZumBewegen = SpielfeldKomponenten.ausgewaehlteZacke.querySelector('.checker.selected');
                    if (steinZumBewegen) {
    ...
    document.querySelectorAll('.point').forEach(zacke => {
        zacke.addEventListener('dblclick', () => {
            if (phase !== "spiel" || this.verbleibendeZuege.length === 0) return;
            // Prüfen, ob dieser Spieler überhaupt schon ausspielen darf
            if (!darfAusspielen(this.aktiverSpielerFarbe)) {
    ...
    // --- HILFSFUNKTION FÜR ZUGBERECHNUNG UND SCHLAGEN --- eig schon in steine reinkopiert muss nur noch richtig verbunden werde 
    /*dann kann man es hier raus löschen
