(() => {
  ...
  var SpielLogik = class {
    constructor(spieler1Objekt, spieler2Objekt) {
      ...
      if (hauptAnzeige && !this.anzeigeSpieler) {
        const aktiveSprache = window.i18n && window.i18n.aktuelleSprache || "de";
        const uebersetzteFarbe2 = typeof farbuebersetzungen !== "undefined" && farbuebersetzungen["Wei\xDF"] ? farbuebersetzungen["Wei\xDF"][aktiveSprache] || "Wei\xDF" : "Wei\xDF";
        if (aktiveSprache === "en") {
          hauptAnzeige.textContent = `Turn: ${this.spieler1.spielerName} (${uebersetzteFarbe2})`;
        } else if (aktiveSprache === "fr") {
          hauptAnzeige.textContent = `Au tour de: ${this.spieler1.spielerName} (${uebersetzteFarbe2})`;
        } else if (aktiveSprache === "es") {
          hauptAnzeige.textContent = `Turno de: ${this.spieler1.spielerName} (${uebersetzteFarbe2})`;
        } else {
          hauptAnzeige.textContent = `Am Zug: ${this.spieler1.spielerName} (${uebersetzteFarbe2})`;
        }
      }
      this.initRollButton();
      this.initPassButton();
    ...
    pruefeSpielerWechsel() {
      if (this.verbleibendeZuege.length === 0) {
        ...
        const aktiveSprache = window.i18n && window.i18n.aktuelleSprache || "de";
        const uebersetzteFarbe2 = typeof farbuebersetzungen !== "undefined" && farbuebersetzungen[this.aktiverSpielerFarbe] ? farbuebersetzungen[this.aktiverSpielerFarbe][aktiveSprache] || this.aktiverSpielerFarbe : this.aktiverSpielerFarbe;
        const farbZusatz = ` (${uebersetzteFarbe2})`;
        if (!this.anzeigeSpieler) {
          this.anzeigeSpieler = document.getElementById("aktueller-spieler");
        }
        if (this.anzeigeSpieler) {
  ...
  function aktualisiereHighscoreAnzeige() {
    ...
    liste.forEach((eintrag) => {
      ...
      const uebersetzteFarbe2 = typeof farbuebersetzungen !== "undefined" && farbuebersetzungen[eintrag.farbe] ? farbuebersetzungen[eintrag.farbe][aktiveSprache] || eintrag.farbe : eintrag.farbe;
      li.innerHTML = `
            <strong style="color: #333;">${eintrag.name}</strong> 
            <span style="font-size: 0.85em; color: #666;">
                (${uebersetzteFarbe2}) - ${eintrag.datum}
            </span>
        `;
